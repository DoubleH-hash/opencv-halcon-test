...
std::vector<torch::Tensor> Yolo::non_max_suppression_v8(torch::Tensor prediction, float confThres, float iouThres)
{
	std::vector<torch::Tensor> output;

	int maxWh = 4096;
	int maxNms = 30000;
	int nc = prediction.sizes()[1] - 4; // classes的个数
	int mi = 4 + nc;
	auto xc = std::get<0>(prediction.slice(1, 4, mi).max(1, false)) > confThres;
	...
	for (int i = 0; i < prediction.size(0); i++)
	{
		...
		// 将第6列的class置信度与第5列的obj置信度相乘
		//x.slice(1, 5, x.size(1)).mul_(x.slice(1, 4, 5));
		// 将预测框坐标从 (x,y,w,h) 格式转换为 (x1,y1,x2,y2) 格式
		torch::Tensor box = xywh2xyxy(x.slice(1, 0, 4));
		std::tuple<torch::Tensor, torch::Tensor> max_tuple = torch::max(x.slice(1, 4, x.size(1)), 1, true);
		x = torch::cat({ box, std::get<0>(max_tuple), std::get<1>(max_tuple) }, 1);
...
cv::cuda::GpuMat Yolo::img2RGB(cv::cuda::GpuMat img) {
	int imgC = img.channels();
	if (imgC == 1)
	{
		cv::cuda::cvtColor(img, img, cv::COLOR_GRAY2RGB);
	}
	else
	{
...
{
	// auto options = torch::TensorOptions().device(torch::kCUDA).dtype(torch::kByte);
	// torch::Tensor data = torch::from_blob(img.data, {(int)height, (int)width, 3 }, options);
...
torch::Tensor Yolo::img2Tensor(cv::cuda::GpuMat img) {
	auto options = torch::TensorOptions().device(torch::kCUDA).dtype(torch::kByte);
	std::vector<int64_t> dims = {img.rows, img.cols, img.channels()};
	long long step = img.step;
	std::vector<int64_t> strides = {step, img.channels(), 1};
	torch::Tensor data = torch::from_blob(img.data, dims, strides, deleter, options);
	data = data.permute({ 2, 0, 1 });
	data = data.toType(torch::kFloat);
	data = data.div(255);
	data = data.unsqueeze(0).to(at::kCUDA);
	return data;
...
std::vector<torch::Tensor> Yolo::prediction(torch::Tensor data)
{
    ...
    try {
		...
		{
			data = pred.toTensor();
			return non_max_suppression_v8(data, confThres, iouThres);
		}
...
std::vector<torch::Tensor> Yolo::prediction(cv::cuda::GpuMat img, int iHeight, int iWidth) {

	ImageResizeData imgRD = resize(img, iHeight, iWidth);
	
	cv::cuda::GpuMat reGpuImg = img2RGB(imgRD.getGpuImg());
	
	 ////cv::Mat reImg;
	 ////reGpuImg.download(reImg);
	 ////torch::Tensor data = img2Tensor(reImg);

	torch::Tensor data = img2Tensor(reGpuImg);
...
std::vector<torch::Tensor> Yolo::prediction(std::vector<cv::cuda::GpuMat> imgs, int iHeight, int iWidth)
{
	std::vector<ImageResizeData> imageRDs;
	std::vector<torch::Tensor> datas;
	for (int i = 0; i < imgs.size(); i++)
	{
		ImageResizeData imgRD = resize(imgs[i], iHeight, iWidth);
		imageRDs.push_back(imgRD);
		cv::cuda::GpuMat img = img2RGB(imgRD.getGpuImg());
		datas.push_back(img2Tensor(img));
	}
	torch::Tensor data = torch::cat(datas, 0);
	//std::cout << "data after cat:" << data.sizes() << std::endl;
	std::vector<torch::Tensor> result = prediction(data);
	return sizeOriginal(result, imageRDs);
}
...
ImageResizeData Yolo::resize(cv::cuda::GpuMat img, int height, int width) {
	ImageResizeData imgResizeData;
	int w = img.cols, h = img.rows;
	imgResizeData.setH(h);
	imgResizeData.setW(w);
	...
	imgResizeData.setWidth(width);
	 ...
	 cv::cuda::resize(img, img, cv::Size(
	 	isW ? width : (int)((float)height / (float)h * w),
	 	isW ? (int)((float)width / (float)w * h) : height));
	 ...
	 {
	 	imgResizeData.setBorder((height - h) / 2);
	 	cv::cuda::copyMakeBorder(img, img, (height - h) / 2, height - h - (height - h) / 2, 0, 0, cv::BORDER_CONSTANT);
	 }
	 else
	 {
	 	imgResizeData.setBorder((width - w) / 2);
	 	cv::cuda::copyMakeBorder(img, img, 0, 0, (width - w) / 2, width - w - (width - w) / 2, cv::BORDER_CONSTANT);
	 }
	imgResizeData.setGpuImg(img);
	return imgResizeData;
}
...
void ImageResizeData::setGpuImg(cv::cuda::GpuMat img) {
	this->gpu_img = img;
}
...
cv::cuda::GpuMat ImageResizeData::getGpuImg() {
	return gpu_img;
}
